...
import { invoices, cashflow, weekly, formatINR } from "@/lib/mock";
    ...
    <AppShell>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard label="Total Revenue" value={totalRevenue} icon={TrendingUp} trend="+18.2%" tone="primary" delay={0} />
          <StatCard label="Pending Amount" value={pending} icon={Clock} trend="+4.1%" tone="warning" delay={80} />
          <StatCard label="Overdue Amount" value={overdue} icon={AlertTriangle} trend="-2.4%" tone="destructive" delay={160} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="rounded-2xl gradient-primary text-primary-foreground p-6 shadow-glow relative overflow-hidden animate-fade-up" style={{ animationDelay: "260ms" }}>
            ...
            <div className="text-4xl font-semibold mt-2 tracking-tight">
              <CountUp to={284600} format={(n) => formatINR(Math.round(n))} />
            </div>
            <p className="text-sm opacity-80 mt-2 leading-relaxed">
              Based on 6 pending invoices and your historical payment cycle of 14 days.
            </p>
            <div className="mt-5 h-2 rounded-full bg-white/15 overflow-hidden">
              <div className="h-full bg-white rounded-full animate-fade-up" style={{ width: "68%", animationDelay: "600ms" }} />
            </div>
            <div className="mt-2 text-xs opacity-80">68% likely to clear on time</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 rounded-2xl bg-card border border-border shadow-card overflow-hidden animate-fade-up" style={{ animationDelay: "380ms" }}>
            ...
            <div className="divide-y divide-border">
              {invoices.slice(0, 5).map((inv) => (
                <div key={inv.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-accent/40 transition-colors group">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate">{inv.id} · Due {new Date(inv.dueDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
                    </div>
                  </div>
